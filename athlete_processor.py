...
uri = "mongodb+srv://colinfitzgerald:N6BnA4O5nmvEATsl@trackathletes.tqfgaze.mongodb.net/?retryWrites=true&w=majority"
...
# Access the "athletes_data" collection
    ...
    for item in split_disciplines: 
        ...
        if len(split_item) > 1: 
            if split_item[1] == "metres": 
                ...
                if re.search("4x\d+", first_item_clean) or int(first_item_clean) <= 3000: 
...
def get_wiki(athlete_name, kw=" track athlete"):
    ...
    response = requests.get('https://en.wikipedia.org/w/index.php', params=params, headers=headers)
    h_t = response.text
    soup = BeautifulSoup(h_t, 'html.parser')
    li_elements = soup.find_all('li', class_='mw-search-result mw-search-result-ns-0')
    track_words_list = ["runner", "track and field", "sprinter", "track", "athlete", "pole vault", "shot put", "high jump", "long jump", "hurdles", "steeplechase", "triple jump", "discus", "racewalker", "javelin", "marathon"]
...
def get_nickname(wiki_url):
    ...
    try: 
        ...
        cleaned_nickname = nickname.replace("<b>", "").replace("</b>", "").replace('"', "")
...
def is_ig_track_athlete(user_name, list_disciplines):
    ...
    formatted_disciplines.extend(["runner", "track and field", "sprinter", "WorldAthletics"])
...
def get_instagram_url_for_athlete(athlete_name, list_disciplines):
    ...
    if len(users) > 0:
        ...
        for user in users: 
            ...
            if len(full_name) > 1: 
                full_name_formatted = convert_special_unicode_to_string(full_name[0].capitalize() + " " + full_name[1].capitalize())
                if is_ig_track_athlete(user_name, list_disciplines) and similarity_percentage(full_name_formatted, athlete_name) > 20: 
                    found_user = user_name
                    break
            elif len(full_name) == 1: 
                full_name_formatted = convert_special_unicode_to_string(full_name[0].capitalize())
                if is_ig_track_athlete(user_name, list_disciplines) and similarity_percentage(full_name_formatted, athlete_name) > 20: 
                    found_user = user_name
                    break
        if found_user: 
...
def get_hq_image_for_athlete(query):
    ...
    response = requests.get(f'https://www.gettyimages.com/photos/{query}', params=params, headers=headers)
...
def return_athletes_with_codes_and_images(athlete_name):
    ...
    for athlete in athletes: 
        ...
        athlete["full_name"] = athlete["givenName"] + " " + athlete["familyName"].lower().capitalize()
        athlete["image_url"] = get_image_for_athlete(athlete["givenName"] + " " + athlete["familyName"] + " " + athlete["country"] + " track and field") 
        suggested_wiki_url = get_wiki(athlete["givenName"] + " " + athlete["familyName"].lower().capitalize())
        if not suggested_wiki_url: 
            suggested_wiki_url = get_wiki(athlete["givenName"] + " " + athlete["familyName"].lower().capitalize(), " sprinter")
...
def get_csv_athletes():
    names = []
    for i in range(1, 6):
        found_names = pd.read_html(f"https://worldathletics.org/world-rankings/overall-ranking/women?regionType=world&page={i}&rankDate=2023-11-21&limitByCountry=0")[0]["Competitor"].tolist()
        names.extend(found_names)
        
    for i in range(1, 6):
        found_names = pd.read_html(f"https://worldathletics.org/world-rankings/overall-ranking/men?regionType=world&page={i}&rankDate=2023-11-21&limitByCountry=0")[0]["Competitor"].tolist()
        names.extend(found_names)
