...
def score_event(gender, event, time_seconds):
    try: 
        ...
        points = (coefs["conversionFactor"] * (time_seconds + coefs["resultShift"])**2 + coefs["pointShift"])
    except TypeError: 
        return "N/A"
    return points 
...
def get_results(competition, date, gender, discipline_code):
    ...
    if not event_id: 
        return {
            "operation": "error", 
            "description": "event not yet supported"
        }
        ...
        if race_results is None: 
            ...
            list_results = race_results["eventTitles"][0]["events"][0]["races"][0]["results"]
...
def return_results_dict(compiled_results):
    ...
    for item in flattened_list: 
        results_dict.append({
            ...
            "score": score_event(item["gender"], item["discipline_code"], time_string_to_seconds(item["mark"]))
        })
    return results_dict
...
def get_top_competitors(profile): 
    time.sleep(10)
    response_data = requests.get(f"https://python.colinfitzgeral8.repl.co/athletes/results?athlete_id={profile['aaAthleteId']}").json()
    results = response_data["athlete_data"]
    name = profile["givenName"] + " " + profile["familyName"]

    compiled_results = get_compiled_results(profile["gender"] + "'s", results)
    ...
    avg_score = actual_results.groupby("name")["score"].mean().reset_index().sort_values(by = "score", ascending = False).set_index("name")

    sum_runs = actual_results.groupby("name").count().sort_values(by = "time_seconds", ascending = False).reset_index()[["name", "time_seconds"]].rename(columns = {"time_seconds": "count"}).set_index("name")
