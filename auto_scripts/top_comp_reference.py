    ...
    if top_competitors:
        if len(top_competitors) > 0: 
            for competitor in top_competitors:
                ...
                projection = {'_id': 0}
                cur = collection.find({"$expr": {"$eq": [{"$concat": ["$givenName", ' ', "$familyName"]}, competitor]}}, projection)
                ...
                if cursor_results:
                    ...
                    top_competitors_with_reference.append({
                      "athlete_name": competitor,
                      "athlete_id": found_reference
                    })

    collection.update_one({"_id": document["_id"]}, {"$set": {"top_competitors_with_reference": top_competitors_with_reference}})
